        ...
        Platform.runLater(new Runnable() {
            ...
            public void run() {
                for (int i= 0; i < albumNames.size(); i++) {
                    final AlbumSearchFeed Album = albumSearchFeed;
                    albumNames.get(i).setText(String.valueOf(Album.results.get(i).collectionName));
                    albumImages.get(i).setImage(new Image(Album.results.get(i).artworkUrl100));
                }
            }
        });
    }
